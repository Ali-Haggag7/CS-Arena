    ...
    return (
        <div className="relative w-full sm:w-auto sm:min-w-[180px] z-50" ref={dropdownRef}>
            <button
                ...
                className="w-full flex items-center justify-between gap-1.5 sm:gap-2 px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl border border-white/10 bg-[#161618] hover:bg-[#1f1f22] transition-colors text-xs sm:text-sm text-white focus:outline-none focus:ring-2 focus:ring-primary/50 shadow-sm"
            >
                <span className="flex items-center gap-1.5 sm:gap-2 truncate text-white/70">
                    {label}
                    <span className="font-semibold text-white truncate max-w-[70px] sm:max-w-[100px]">
                        {selectedOption ? selectedOption.name : allLabel}
                    </span>
                </span>
                <ChevronDown className={`size-3 sm:size-4 shrink-0 text-white/50 transition-transform ${isOpen ? "rotate-180 text-primary" : ""}`} />
            </button>
